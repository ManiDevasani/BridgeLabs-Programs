...
class LinkedList:
    ...
    def indexOf(self, data):
        ...
        #  taking the local variable as 0 and compare the that variable to size every time
    ...
    def insertOrder(self, data):
        # inserting the element at specified position to follow the ascending order
        # creating  node class for creating node and passing data to it
        node = Node(data)
        node.data = data
        node.next = None
        n = self.head
        # to traverse until size-1 and every time checking the next element is greater than the given element
        # if the given element is greater than the next element then it will add the given element at before that
        while n.next.next is not None:
            if n.next.data > data:
                n1 = n.next
                n.next = node
                node.next = n1.next
        # if the element is present at size-1 position then placing the given node before that
        if n.next.data > data:
            n1 = n.next
            n.next = node
            node.next = n1.next
        else:
            # if the none of the elements in the list is greater than the given number then adding it last to the list
